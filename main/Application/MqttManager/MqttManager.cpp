...
void MqttManager::Init()
{
    ...
    if (prefix[0] == '\0')
        snprintf(prefix, sizeof(prefix), "spectry");
                             ...
                             const char *deviceName, const char *version)
{
    ...
    json.fieldArray("ids");
    char id[32];
    snprintf(id, sizeof(id), "spectry_%s", deviceId);
    json.value(id);
    json.endArray();

    json.field("name", deviceName);
    json.field("mf", "Spectry");
                                          ...
                                          std::function<void(JsonWriter &)> writeFields)
{
    ...
    if (deviceName[0] == '\0')
        snprintf(deviceName, sizeof(deviceName), "Spectry");
    ...
    char uid[64];
    snprintf(uid, sizeof(uid), "spectry_%s_%s", deviceId_, objectId);
...
void MqttManager::PublishDiscovery()
{
    ...
    for (const auto &s : sensors)
    {
        ...
        char uid[64];
        snprintf(uid, sizeof(uid), "spectry_%s_%s", deviceId_, s.objectId);
        json.field("uniq_id", uid);
...

    {
        ...
        char uid[64];
        snprintf(uid, sizeof(uid), "spectry_%s_reboot", deviceId_);
        json.field("uniq_id", uid);
